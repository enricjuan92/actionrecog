...
def video_to_tensor(nparray):
    # video [channels, frames, height, width] vs tensor [frames, height, width, channels]
    return tf.reshape(nparray, [nparray.shape[1], nparray.shape[2], nparray.shape[3], nparray.shape[0]])

def video_extractor(path):
    print ('Hola 1')
    video = video_to_array(video_path=path, resize=[224, 224], start_frame=0, end_frame=2)
    print('Hola 2')
    print(video)
    return video
